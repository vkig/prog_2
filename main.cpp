...
void lehetosegek_general(map<string, set<vector<pair<char, set<string>>>>> forras)
{
    vector<pair<char,string>> egy_kocsi_egy_lehetosege;
    auto kocsi_lehetosegek_set=*forras.begin();
    {
        for(auto kocsi_egy_lehetoseg_sorozata_tobb_kimenetel:kocsi_lehetosegek_set.second)
        {
            for(auto p:kocsi_egy_lehetoseg_sorozata_tobb_kimenetel)
            {
                for(auto b:p.second)
                {
                    egy_kocsi_egy_lehetosege.push_back(pair<char, string>(p.first,b));
                    cout<<p.first<<" "<<b<<endl;
                }
            }
        }
...
struct vasuti_halozat_allapot
{
    ...
    vector<vasuti_halozat_allapot> gyerekek_general()
    {
        ...
        lehetosegek_general(lehetseges_akciok);
