        ...
        for(auto k:kocsik)
        {
            ...
            if(k.second.csatlakoztatva_e())
            {
                ...
                else
                {
                    ...
                    {
                        if(nem_ertem.find(a.first)!=nem_ertem.end() && a.second>(allomasok[d.get_vonat_allomas(k.second.get_hely(),elozmenyek.size())].get_termek(a.first)))
                        {
                            kell=false;
                    ...
                    //JAR ITT MASIK VONAT IS
                    if(kell && (d.keresztul_megy_e_masik((d.get_vonat_allomas(k.second.get_hely(),elozmenyek.size())))))
                    {
                        //lecsatol
                        lehetseges_akciok[k.first].insert(vector<pair<char, string>> {pair<char, string>('a', "0")});
                        //lecsatol-felcsatol=ATCSATOL
                        if(d.get_vonatok_allomason((d.get_vonat_allomas(k.second.get_hely(),elozmenyek.size())), elozmenyek.size()).size()>1)
                        {
                            for(auto p:(d.get_vonatok_allomason((d.get_vonat_allomas(k.second.get_hely(),elozmenyek.size())), elozmenyek.size())))
                            {
                        ...
                        //lecsatol-felpakol
                        if(((d.get_kocsi_arukapacitasa(k.first)-k.second.get_toltottseg())>0) && (allomasok[(d.get_vonat_allomas(k.second.get_hely(),elozmenyek.size()))].termek_mennyiseg()>0))
                        {
                            for(auto a:(allomasok[(d.get_vonat_allomas(k.second.get_hely(),elozmenyek.size()))].get_termekek()))
                            {
                                lehetseges_akciok[k.first].insert(vector<pair<char, string>>({pair<char, string>('a',"0"), pair<char, string>('d', a.first)}));
                        ...
                        //lecsatol-lepakol -- NEM TUDOM MIK LEGYENEK -  további feltételek kellenének, mert alapból a lepakolásnak nem sok értelme, ahhoz több termék kell, vagy kevés cucc nagy kocsiban
            ...
            else
            {
                ...
                //felpakol
                if( ((d.get_kocsi_arukapacitasa(k.first)-k.second.get_toltottseg())>0) && (allomasok[k.second.get_hely()].termek_mennyiseg()!=0))
                {
